...
- **`packages/acits_l10n`** — a single generated easy_localization `LocaleKeys`
  (source: `assets/translations/*.json`, regenerated by `melos run l10n`).
  The app and every module import it, so there is one source of translation
  keys and no per-module duplication.
