...
> **Physical modularization (post-9, follow-up):** the group migrations moved features onto the new stack *logically* but left screens in `lib/`. A follow-up sweep extracted every feature into its own `modules/<feature>` package matching the `animals` reference: shared widgets → `acits_ui_kit` (957814e), then `modules/{auth (463f480), applicants (2ffee25), prescriptions (944e498), personal (e584341), media (75614b4)}`. Each: barrel, module domain contracts + ports (session/shelter/type-label/notes/animal-loader/doc-exporter bridged in the app), constructor injection, **zero `package:acits_flutter` imports**, tests moved in. App imports features only via barrels; `animal_detail`/`animal_edit`/`main`/`root` stay in the shell (composition point; hosting them in a module would form a `media→animals→media` cycle). 6 feature modules total; 294 tests green; strict analyze + prod build web ✓.
