...
| --- | --- | --- |
| `InvalidType`, "the getter/field X isn't defined", missing DTO field after a spec change | Stale API client — the OpenAPI spec changed but `melos genapi` was not run | `melos genapi` (regenerates the swagger_parser client inside `acits_api`) |
| New `@JsonSerializable` DTO won't compile / `_$FooFromJson` undefined | `.g.dart` not generated | `melos gen` (or `MELOS_GENONE_PKG=packages/acits_api melos run genone`) |
...
| `build_runner` fails inside `acits_api` with dangling imports around `Species` / `Applicant` / `Prescription` etc. | swagger_parser 1.44 drops data classes used as `multipart/form-data` request bodies | this is why `melos genapi` runs `tool/preprocess_openapi.dart` first — always go through `melos genapi`, never raw `swagger_parser`. See [`packages/acits_api/README.md`](../packages/acits_api/README.md) |
...
| Analyzer flags files under `packages/*/build/` or generated output | those are build artifacts | `packages/*/build/`, `packages/*/.dart_tool/`, `modules/*/build/` are gitignored; generated code is excluded from analysis — don't commit `build/` |
...
| Symptom | Likely cause | Fix |
| --- | --- | --- |
| Compile error importing `dart:html` | forbidden | use `package:web` + `dart:js_interop` behind a conditional-import port in `acits_core` |
