...
void main() {
  group('AuthApiAdapter parity — login', () {
    test('raw POST /api/token/ sends only credentials, maps access/refresh onto TokenPairDto', () async {
  ...
  group('AuthApiAdapter parity — register', () {
    test('admin write DTO serialises onto the generated body; response maps to UserAdminDto', () async {
      ...
      // Write body carried through to the generated model. `shelter` is now
      // nullable in the generated model (global spec relax), but the write
      // body always carries it — assert on the non-null value.
