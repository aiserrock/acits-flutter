...
import 'package:auth/auth.dart' show AuthDeepLinkHandler;
...
@singleton
class DeepLinkService implements Disposable, AuthDeepLinkHandler {
  DeepLinkService() {
  ...
  void onLinkHandle(String link) {
    ...
    if (_isEmailVerifyLink(link)) {
      getIt<GoRouter>().push('${Routes.emailConfirmation}?link=${Uri.encodeComponent(link)}');
    }
  }
