...
class _SearchDrugScreenState extends State<SearchDrugScreen> {
  ...
  final _service = getIt<PrescriptionService>();
  ...
  final _drugsState = WidgetState<List<ShelterDrug>>()..loading();
  ...
  Widget _buildBody() {
    return StateBuilder<List<ShelterDrug>>(
      state: _drugsState,
      builder: _buildList,
  ...
  Widget _buildList(context, List<ShelterDrug> list) {
  ...
  Widget _buildItemTitle(ShelterDrug item) {
    final drugForm = item.drug?.formOfDrugName;
    return Text.rich(
      TextSpan(
        children: [
          TextSpan(
