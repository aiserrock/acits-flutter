...
/// Экран поиска / выбора приюта
class SearchShelterScreen extends StatefulWidget {
  const SearchShelterScreen({
    Key? key,
  }) : super(key: key);

  @override
  State<SearchShelterScreen> createState() => _SearchShelterScreenState();
}

class _SearchShelterScreenState extends State<SearchShelterScreen> {
  ...
  final _drugsState = WidgetState<List<ShelterShortSerializers>>()..loading();
  ...
  Widget _buildBody() {
    return StateBuilder<List<ShelterShortSerializers>>(
      state: _drugsState,
      builder: _buildList,
  ...
  Widget _buildList(context, List<ShelterShortSerializers> list) {
  ...
  Widget _buildItemTitle(ShelterShortSerializers item) {
    return Text.rich(
      TextSpan(
        children: [
          TextSpan(
  ...
  Future<void> _loadData({bool resetOffset = false}) async {
    ...
    _service
      ...
      (value) {
        setState(
          () {
            ...
            final list = (_drugsState.value ?? [])..addAll(value?.results ?? []);
            _drugsState.content(list);
      ...
      (error, stackTrace) {
        if (_drugsState.isLoading) {
          setState(() => _drugsState.error = error);
        } else {
